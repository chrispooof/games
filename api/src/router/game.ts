...
export const gameRouter = router({
  ...
  create: publicProcedure.input(createGameInputSchema).mutation(async ({ input }) => {
    try {
  ...
  get: publicProcedure.input(getGameInputSchema).query(async ({ input }) => {
    try {
      const game = await getGame(input.roomCode)
      if (!game) {
        throw new TRPCError({
          code: "NOT_FOUND",
          message: `Game ${input.roomCode} not found`,
        })
      }
      return {
        roomCode: game.roomCode,
